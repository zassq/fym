...
(g=c.Deferred(),this._slots.push(g),k=g.pipe(l)):k=this._sequence=this._sequence.pipe(l,l),k.abort=function(){f=[void 0,"abort","abort"];return e?e.abort():(g&&g.rejectWith(h.context,f),l())},this._enhancePromise(k)):l()},_onAdd:function(a,b){var d=this,e=!0,f=c.extend({},this.options,b),g=b.files,k=g.length,h=f.limitMultiFileUploads,l=f.limitMultiFileUploadSize,p=f.limitMultiFileUploadSizeOverhead,m=0,n=this._getParamName(f),q,r,s=0;!l||k&&void 0!==g[0].size||(l=void 0);if((f.singleFileUploads||
h||l)&&this._isXHRUpload(f))if(f.singleFileUploads||l||!h)if(!f.singleFileUploads&&l)for(r=[],q=[],f=0;f<k;f+=1){if(m+=g[f].size+p,f+1===k||m+g[f+1].size+p>l)r.push(g.slice(s,f+1)),m=n.slice(s,f+1),m.length||(m=n),q.push(m),s=f+1,m=0}else q=n;else for(r=[],q=[],f=0;f<k;f+=h)r.push(g.slice(f,f+h)),m=n.slice(f,f+h),m.length||(m=n),q.push(m);else r=[g],q=[n];b.originalFiles=g;c.each(r||g,function(f,g){var h=c.extend({},b);h.files=r?g:[g];h.paramName=q[f];d._initResponseObject(h);d._initProgressObject(h);
