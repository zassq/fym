...
(function($) {
	...
	var ColumnFilterWidgets = function( oDataTableSettings ) {
		...
		if ( 'oColumnFilterWidgets' in oDataTableSettings.oInit ) {
			...
			if ( 'bGroupTerms' in oDataTableSettings.oInit.oColumnFilterWidgets && oDataTableSettings.oInit.oColumnFilterWidgets.bGroupTerms ) {
				me.$MenuContainer = $( '<div class="column-filter-widget-menus clearfix"></div>' );
				me.$TermContainer = $( '<div class="column-filter-widget-selected-terms col-xs-12 col-sm-12 col-md-12"></div>' ).hide();
			}
		}
		...
		if ( me.$TermContainer ) {
            me.$TermContainer.prepend('<p><strong>过滤条件（单击去除已选条件）：</strong></p>')
            me.$WidgetContainer.append('<div class="col-md-12"><h3>过滤</h3></div>');
	...
	*/
	var ColumnFilterWidget = function( $Container, oDataTableSettings, i, widgets ) {
		...
		widget.$Select = $( '<select class="form-control input-sm"></select>' ).change( function() {
